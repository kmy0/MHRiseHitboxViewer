...
local function options()
    ...
    if imgui.tree_node(gui_util.tr("mod.tree_damage_type")) then
        generic.draw_box_type_options(
            ...
            function(t, i, _)
                return e.get("snow.hit.DamageType")[t[i]] ~= nil
            end
        )
        imgui.tree_pop()
    end

    if imgui.tree_node(gui_util.tr("mod.tree_hit_condition")) then
        generic.draw_box_type_options(
            util_table.keys(config_mod.hitboxes.hit_condition.disable),
            "mod.hitboxes.hit_condition",
            function(t, i, _)
                return e.get("snow.hit.AttackConditionMatchHitAttr")[t[i]] ~= nil
            end
        )
        imgui.tree_pop()
    end
