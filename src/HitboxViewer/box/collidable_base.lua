...
function this:new(collidable, parent, box_type, resource_idx, set_idx, collidable_idx)
    ...
    local userdata = collidable:get_UserData() --[[@as snow.hit.userdata.CommonAttachShapeData]]
    local custom_shape = userdata._CustomShapeType
    ---@type string?
    local shape_name

    if custom_shape and custom_shape ~= e.get("snow.hit.CustomShapeType").None then
        shape_name = e.get("snow.hit.CustomShapeType")[custom_shape]
    end

    if not shape_name then
        shape_name = e.get("via.physics.ShapeType")[shape:get_ShapeType()]
    end
    ...
    o.ptr_userdata = o.userdata:get_address()
    return o
end
...
function this:update_shape()
    ...
    if self.is_enabled then
        if
            ...
            or self.shape_type == mod_enum.shape.Donuts
        then
        ...
        if self.shape_type == mod_enum.shape.Donuts then
            self.shape_data.ring_radius = hb_draw.read_float(self.ptr_userdata + 0xA4)
        end
