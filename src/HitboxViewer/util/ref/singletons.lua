---@class SingletonUtil
---@field singletons table<string, snow.SnowSingletonBehaviorRoot | NativeSingleton>
...
function this.get(singleton)
    if not this.singletons[singleton] then
        this.singletons[singleton] = sdk.get_managed_singleton(singleton) --[[@as snow.SnowSingletonBehaviorRoot]]
    end
